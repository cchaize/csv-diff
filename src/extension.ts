...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('csvDiffViewer.showDiff', async (item: CsvFileItem) => {
...
class CsvDiffProvider implements vscode.TreeDataProvider<CsvFileItem> {
    private _onDidChangeTreeData = new vscode.EventEmitter<CsvFileItem | undefined | void>();
        ...
        for (const repo of git.repositories) {
            ...
            for (const change of changes) {
                if (change.uri.fsPath.endsWith('.csv')) {
                    modifiedCsvFiles.push(new CsvFileItem(
